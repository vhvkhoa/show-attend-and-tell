...
flags.DEFINE_integer('lstm_hidden_dim', 1536, 'Hidden layer size for LSTM cell.')
flags.DEFINE.integer('time_steps', 31, 'Number of time steps to be iterating through.')
...
flags.DEFINE_integer('eval_steps', 200, 'Evaluate and save current model every eval_steps steps.')
flags.DEFINE_integer('metric', 'CIDEr', 'Metric being based on to choose best model, please insert on of these strings: [Bleu_i, METEOR, ROUGE_L, CIDEr] with i is 1 through 4.')
...
def main():
    ...
    model = CaptionGenerator(word_to_idx, dim_feature=[FLAGS.image_feature_size, FLAGS.image_feature_depth], dim_embed=FLAGS.embed_dim,
                                    lstm_hidden_size=FLAGS.lstm_hidden_size, lstm_num_layer=FLAGS.lstm_layers, 
                                    n_time_step=FLAGS.time_steps, prev2out=FLAGS.prev2out,
